...
class LU():
        ...
        for i in range(self.n):
            ...
            for k in range(i, self.n): 
                if ~np.isclose(self.U[i, i], 0.0):
                    break
                self.U[[k, k+1]] = self.U[[k+1, k]]
                self.P[[k, k+1]] = self.P[[k+1, k]]
    ...
    def inverse(self):
        ...
        Ainv = self.solve(i_mat).transpose()
    ...
    def det(self):
        ...
        if self.number_of_permutations % 2 == 0:
            sol = (np.diagonal(self.U.mat).prod())*(np.diagonal(self.L.mat).prod())
            #sol = (np.diagonal(self.U).prod())*(np.diagonal(self.L).prod())
            return sol
        else:
            sol2 = (-1)*(np.diagonal(self.U.mat).prod())*(np.diagonal(self.L.mat).prod())
            return sol2
...
ls1 = LU(np.array([[1,1,-1], [2,0,1], [0,1,1]], np.double))
ls2 = LU(np.array([[0, 3, 4], [4, 6, 10], [22, 1, 7]]))
#c = np.array([0, 14, 13])
#ls1.solve(c)
#print(self.U)
#ls1.solve(c)
